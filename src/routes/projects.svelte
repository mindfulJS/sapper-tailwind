...
{#each projects.list as project, i}
  <div transition:slide="{{ delay: 20 }}" class="max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl flex mx-auto p-6 m-6 bg-white rounded-lg shadow-xl" on:mouseover="{() => project.showDescription = true}" on:mouseout="{() => project.showDescription = false}">
